...
- 安装后命令全名为 `sdlc-agent-pipeline:requirement` 等；`/code`、`/test` 这类短名可能与其他插件或项目命令撞名，撞名时用全名。
...
1. **交互模式**：团队成员在本地 Claude Code 里安装本插件，手动执行 `/requirement` `/design` `/code` `/test` 或一键 `/pipeline`。
