...
- **会话启动即知进度**：SessionStart 钩子在每次会话启动时运行 `status --brief`，把 feature、各阶段确认情况与下一步建议注入会话上下文——新开窗口不用再手动查状态；无状态文件时零输出（不干扰非编排模式），状态损坏时提示修复但不阻塞会话。
...
- **superpowers**（github.com/obra/superpowers）：技能自动触发的「强制工作流」（Mandatory workflows, not suggestions）与「evidence over claims」哲学——本插件的定稿机器校验（validate-handoff.js）、SessionStart 状态引导与脚本单元测试纪律是这一哲学的落地。
- **mattpocock/skills**（github.com/mattpocock/skills）：小而自包含的 skill（SKILL.md + 捆绑资源）与沉淀式工作流（grill 澄清沉淀文档、决策记录）——本插件的 skill 目录形态、澄清笔记与「关键决策」节借鉴于此。
...
5. **钩子配置注记**：`hooks/hooks.json` 顶层 `description` 为可选字段；`timeout` 单位为秒。钩子在会话启动时加载——修改 `hooks/hooks.json` 或新增钩子脚本后，需重启 Claude Code 会话才生效（可用 `/hooks` 查看当前已加载的钩子）。
6. **改完脚本跑测试**：`node --test`（`tests/` 下基于 `node:test` 的零依赖测试，覆盖门禁、状态管理与交接块校验的全部分支，并永久锁定交接块示例与格式约定的一致性）。
