...
- 目录速查：`agents/` 角色定义 · `rules/` 技术栈规约 · `skills/` 共享 SOP（各自带 `scripts/`/`examples/`：状态管理在 baseline-gate、交接块校验在 context-handoff）· `commands/` 斜杠命令 · `hooks/` 阶段门禁与状态注入 · `templates/` 文档与代码模板 · `tests/` 脚本测试（`node --test`）· `CHANGELOG.md` 变更日志
